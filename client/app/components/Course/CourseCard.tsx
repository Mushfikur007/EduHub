...
const CourseCard: FC<Props> = ({ item, isProfile }) => {
  ...
  const handleImageError = () => {
    console.log("Image failed to load:", item?.thumbnail?.url);
    setImgError(true);
  };

  // For debugging
  console.log("Course:", item?.name, "Thumbnail URL:", item?.thumbnail?.url);
  ...
  return (
    ...
    >
      <div className="w-full min-h-[35vh] dark:bg-slate-500 dark:bg-opacity-20 backdrop-blur border dark:border-[#ffffff1d] border-[#00000015] dark:shadow-[bg-slate-700] rounded-lg p-3 shadow-sm dark:shadow-inner">
        <img
          src={imgError ? "/assets/business-img.png" : item?.thumbnail?.url}

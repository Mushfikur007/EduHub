...
const CourseContentList: FC<Props> = (props) => {
  ...
  if (!props.data || !Array.isArray(props.data) || props.data.length === 0) {
    return <div className="w-full text-center">No course content available</div>;
  }

  // Find unique video sections
  const videoSections: string[] = [
    ...new Set<string>(props.data.map((item: any) => item.videoSection || "Untitled Section")),
  ];
  ...
  // Safe function to handle video click
  const handleVideoClick = (videoIndex: number) => {
    if (!props.isDemo && typeof props.setActiveVideo === 'function') {
    ...
    <div className={`mt-[15px] w-full ${!props.isDemo && 'ml-[-30px] min-h-screen sticky top-24 left-0 z-30'}`}>
      {videoSections.map((section: string, sectionIndex: number) => {
        ...
        const sectionVideos: any[] = props.data.filter(
          (item: any) => (item.videoSection || "Untitled Section") === section
        );
            ...
            {isSectionVisible && (
              <div className="w-full">
                {sectionVideos.map((item: any, index: number) => {
                  ...
                  const contentLength: number = (Number(item.videoLength) || 0) / 60;
                  return (
                    <div
                      className={`w-full ${
